...
class CausalConvTranspose1d(nn.Module):
    def __init__(self, in_chan, hid_chan, kernel_size, stride=1, padding=0, output_padding=0):
        super(CausalConvTranspose1d, self).__init__()
        self.conv_transpose = nn.ConvTranspose1d(in_chan, hid_chan, kernel_size, stride=stride, padding=padding, output_padding=output_padding)

    def forward(self, x):
        # 获取卷积核大小
        kernel_size = self.conv_transpose.kernel_size[0]
        # 在时间维度上添加足够的填充，以保证转置卷积是因果的
...
# 添加split_feature和merge_feature函数
def _pad_segment(input, segment_size):
    ...
    if rest > 0:
        pad = torch.zeros(batch_size, dim, rest, device=input.device, dtype=input.dtype)
        input = torch.cat([input, pad], 2)

    pad_aux = torch.zeros(batch_size, dim, segment_stride, device=input.device, dtype=input.dtype)
...
def split_feature(input, segment_size):
    ...
    # input is the features: (B, C, T)  #(B, N, T)
...
class SingleLSTM(nn.Module):
    ...
    参数：
        input_size: 输入特征维度，形状应为 (batch, seq_len, input_size)。
        hidden_size: 隐藏层维度。
...
class SegLSTM(nn.Module):
    ...
    参数：
        rnn_type: RNN类型，如'LSTM'。
        in_chan: 输入特征维度。
        hidden_size: 隐藏层维度。
        dropout: Dropout比例，默认0。
        bidirectional: 是否使用双向LSTM，默认False。
        norm_type: 归一化类型，如'gLN'或'cLN'。
    """

    def __init__(
        self, rnn_type, in_chan, hidden_size, dropout=0.0, bidirectional=False, norm_type="cLN3d"
    ):
        super().__init__()
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.proj = nn.Linear(hidden_size * self.num_direction, in_chan)
...
class MemLSTM(nn.Module):
    ...
    参数：
        ...
        dropout: Dropout比例，默认0。
        bidirectional: 是否使用双向LSTM，默认False。
        mem_type: 控制SegLSTM的隐藏或细胞状态是否由MemLSTM处理，可选'hc'、'h'、'c'或'id'。
        norm_type: 归一化类型，如'gLN'或'cLN3d'。
    """
    ...
    def __init__(
        ...
        assert mem_type in [
            ...
            "c",
            "id",
        ], f"仅支持 'hc'、'h'、'c' 和 'id'，当前类型: {mem_type}"
        ...
        if mem_type in ["hc", "h"]:
            ...
            self.h_norm = normalizations.get(norm_type)(
                self.input_size
            )
        if mem_type in ["hc", "c"]:
            ...
            self.c_norm = normalizations.get(norm_type)(
                self.input_size
            )

    def forward(self, hc, S):
        # hc = (h, c)，来自SegLSTM的隐藏和细胞状态元组
        # h和c的形状: (d, B*S, H)
        # S: SegLSTM中的段数
...
# Cache Memery 
class NewDualPathRNN(nn.Module):
    def __init__(
        self,
        in_chan: int,
        hid_chan: int,
        ...
        super(NewDualPathRNN, self).__init__()
        self.in_chan = in_chan
        self.hid_chan = hid_chan
        self.kernel_size = kernel_size  # 分段大小
        self.stride = stride
        ...
        # 定义处理时间/频率维度的SegLSTM和MemLSTM
        self.seg_lstm = SegLSTM(
    ...
    def forward(self, x: torch.Tensor, hc):
        ...
        x = F.pad(x, (0, new_T - old_T, 0, new_F - old_F))

        #处理T
        residual = x.transpose(-2, -1).contiguous()  # B, C, new_T, new_F
        x = self.norm(x)
        x = (
            x.permute(0, 3, 1, 2).contiguous().view(B * new_F, C, new_T)
        )  #  [B * new_F, C, T]
        x, rest = split_feature(x, segment_size=self.segment_size)  #[B * new_F, C, K(segment_size), S(segments片段数)]
        x = x.permute(0, 3, 2, 1).contiguous()  # [B*F, S, K, C]
        B_F, _S, _K, _D = x.shape
        x = x.view(B_F * _S, _K, _D).contiguous()  # [B*F*S, K, C]

        # 通过SegLSTM处理
        x, hc = self.seg_lstm(x, hc)       
        x = x.view(B_F, _S, _K, _D).permute(0, 3, 2, 1).contiguous()  # (B*F, C, K, S)
        x = merge_feature(x, rest)    # B*F, C, T     
        x = self.inter_linear(x)[..., :new_T]     # [B*F, C, T]     ############################################
        x = x.view(B, new_F, C, new_T)  # [B, new_F, C, new_T]
        x = x.permute(0, 2, 3, 1).contiguous()  # [B, C, new_T, new_F]
        x = x + residual  # [B, C, new_T, new_F]
        x = x[..., :old_T, :old_F]
        return x, hc, _S
...
class AudioBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, hc=None, index=True):
        # x: B, C, T, (F)
        ...
        '''
        #新增
        global_features, hc, _S = self.process_t(global_features, hc)
        global_features = self.process_f(global_features)
        global_features = self.process_att(global_features)
        if index:
            hc = self.process_mem(hc, _S)  
...
class Separator(nn.Module):
    ...
    def forward(self, audio: torch.Tensor, video: torch.Tensor):
        ...
        for i in range(self.fusion_repeats):
            if self.audio_shared:
                audio, hc = self.audio_net(audio + audio_residual if i > 0 else audio, hc)
            else:
                audio, hc = self.audio_net[i](audio + audio_residual if i > 0 else audio, hc)
            if self.video_shared:
        ...
        for j in range(self.audio_repeats):
            if j < self.audio_repeats - 1:
                index = True
                i = j + self.fusion_repeats
                if self.audio_shared:
                    audio, hc = self.audio_net(audio + audio_residual if i > 0 else audio, hc, index)
                else:
                    audio, hc = self.audio_net[i](audio + audio_residual if i > 0 else audio, hc, index)
            else:
                index = False                
                i = j + self.fusion_repeats
                if self.audio_shared:
...
class RTFSNetCausalNewDPRNN_TF(BaseModel):
    def __init__(
        self,
        n_src: int,
        enc_dec_params: dict,
        ...
        super(RTFSNetCausalNewDPRNN_TF, self).__init__(sample_rate=sample_rate)
        self.n_src = n_src
